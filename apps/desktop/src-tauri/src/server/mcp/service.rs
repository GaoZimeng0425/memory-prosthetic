    ...
    pub async fn handle_list_tools(&self, _params: Option<Value>) -> Result<Value, McpModuleError> {
        Ok(serde_json::json!({
            "tools": [
                {
                    "name": "search",
                    "description": "在 Memory Prosthetic（简称 MP）中搜索已收集的内容。支持关键词搜索和自然语言查询（如'搜索 react 收藏夹'），自动提取搜索意图并返回相似度排序的结果。支持按收藏夹、标签、状态筛选。",
                    "inputSchema": {
                        "type": "object",
                        "properties": {
                            "query": {
                                "type": "string",
                                "description": "搜索关键词或自然语言指令（如'搜索 react 收藏夹'）"
                            },
                            "limit": {
                                "type": "integer",
