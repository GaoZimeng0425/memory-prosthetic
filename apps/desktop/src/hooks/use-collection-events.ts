...
export function useCollectionEvents(): void {
  ...
  useEffect(() => {
    ...
    const unlistenPromise = listen<CollectionEventType & Record<string, unknown>>(
      'collection-event',
      async (event) => {
          ...
          case 'Deleted': {
            ...
            queryClient.setQueriesData(
              ...
              (oldData: unknown) => {
                ...
                return {
                  ...oldData,
                  data: collections.filter((c: unknown) => typeof c === 'object' && c !== null && 'id' in c && (c as { id: number }).id !== id),
                }
              }
            )
            // Invalidate stats
