...
export function ClusterCard({ cluster, color, isSelected = false, onSelect }: ClusterCardProps) {
  ...
  const connectedNodes = cluster.nodeIds.length
  ...
  return (
    <Card
      ...
      style={isSelected ? { ringColor: color } : undefined}
    >
        ...
        <div className="flex items-center justify-between rounded bg-slate-50 px-2 py-1.5">
          ...
          <div className="flex items-center gap-2">
            <div className="h-1.5 w-16 rounded-full bg-slate-200">
              <div className="h-full rounded-full bg-blue-500" style={{ width: `${cluster.density * 100}%` }} />
            </div>
            <span className="font-medium text-xs w-8 text-right">{densityPercent}%</span>
          </div>
        </div>
        ...
        {isExpanded && (
          <div className="mt-3 space-y-2 border-t pt-3">
            <div className="text-xs">
              <p className="mb-2 font-medium text-slate-600">包含节点 (ID):</p>
